...
function ReelItem({ reel, author, meId, active, muted, onToggleMute, onLike, onDelete, onOpenAuthor, onOpenComments, itemHeight }) {
  ...
  const nativePlayerHeight = width * (9 / 16); // the fixed 16:9 shape YouTube's embed always renders at
  const coverScale = itemHeight / nativePlayerHeight; // scale that box up until it covers the full screen
  ...
  return (
    <View style={{ height: itemHeight, width: '100%', backgroundColor: '#000' }}>
      {reel.isExternal ? (
        active ? (
          <View style={{ width, height: itemHeight, overflow: 'hidden', alignItems: 'center', justifyContent: 'center' }}>
            {/* YouTube's embedded player always renders at a fixed 16:9 shape no matter what
                size we ask for — so we render it at that natural size, then zoom the whole
                thing up until it covers the full vertical screen, matching how our own videos fill it. */}
            <View style={{ width, height: nativePlayerHeight, transform: [{ scale: coverScale }] }}>
              <YoutubePlayer
                height={nativePlayerHeight}
                width={width}
