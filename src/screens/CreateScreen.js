...
// mode = 'post' (photos only) | 'story' (photo or video, disappears after 24h)
export default function CreateScreen({ route, navigation }) {
  const isStory = route.params?.mode === 'story';
  const { me } = useAuth();
  ...
  const pick = async (source) => {
    try {
      const a = await pickMedia({ source, video: isStory, square: !isStory });
      if (a) setAsset(a);
    } catch (e) {
  ...
  const choose = () =>
    Alert.alert(isStory ? 'Add to story' : 'New post', 'Choose a source', [
  ...
  const boxW = isStory ? width * 0.62 : width - 40;
  const boxH = isStory ? boxW * 1.7 : boxW;

  return (
    <Screen edges={['top', 'bottom']}>
      <ScreenHeader title={isStory ? 'New story' : 'New post'} onBack={() => !uploading && navigation.goBack()} />
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={{ padding: 20, gap: 16, alignItems: 'center' }} keyboardShouldPersistTaps="handled">
          ...
          >
            {asset ? (
              ...
              <View style={{ alignItems: 'center', gap: 8 }}>
                ...
                <T weight="medium">{isStory ? 'Choose a photo or video' : 'Choose a photo'}</T>
              </View>
            )}
          </Pressable>
          ...
          <Btn
            ...
            label={isStory ? 'Share to story' : 'Share post'}
            onPress={share}
          />
